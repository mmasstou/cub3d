...
void	rendering_walll(t_data *data, t_rays *rays, int ray_id)
{
	...
	// if (!rays->hit)
	// 	return; 
	ray_distance = rays->distance * cos(rays->angle - data->player->rotation_angle);
	distance_project_plane = ((W / 2) / tan(FOV / 2));
	rays->wall_strip_height = ((data->unit / ray_distance) * distance_project_plane);
	data->player->y_start_point = (H / 2) - (rays->wall_strip_height / 2);
	if (data->player->y_start_point < 0)
		data->player->y_start_point = 0;
